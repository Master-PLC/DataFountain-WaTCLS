...
class Backbone(nn.Module):
    ...
    def forward(self, images):
        ...
        # Shape: [batch_size, ch1, h1, w1] = [32, 64, 288, 288]
        x = self.base_network.conv1(images)
        x = self.base_network.bn1(x)
        x = self.base_network.relu(x)
        # Shape: [batch_size, ch1, h2, w2] = [32, 64, 144, 144]
        x = self.base_network.maxpool(x)
        # Shape: [batch_size, ch2, h2, w2] = [32, 256, 144, 144]
        x = self.base_network.layer1(x)
        # Shape: [batch_size, ch3, h3, w3] = [32, 512, 72, 72]
        x = self.base_network.layer2(x)
        # Shape: [batch_size, ch4, h4, w4] = [32, 1024, 36, 36]
        x = self.base_network.layer3(x)
        # Shape: [batch_size, ch5, h5, w5] = [32, 2048, 18, 18]
