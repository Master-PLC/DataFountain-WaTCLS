...
class LossLogger:
    ...
    def log_losses(self, file_name, epoch, loss, metrics, loss_unk=''):
        ...
        log_file.write(str(epoch)+','+str(loss)+',' +
                       str(loss_unk)+','+str(metrics['mAP'])+'\n')
        log_file.close()
...
class Logger:
    def __init__(self, args):
        ...
        if args.model_name:
            ...
            self.file_names = {}
            self.file_names['train'] = os.path.join(
                args.model_name, 'train_results.csv')
            self.file_names['valid'] = os.path.join(
                args.model_name, 'valid_results.csv')
            self.file_names['test'] = os.path.join(
                args.model_name, 'test_results.csv')

            self.file_names['valid_all_aupr'] = os.path.join(
                args.model_name, 'valid_all_aupr.csv')
            self.file_names['valid_all_auc'] = os.path.join(
                args.model_name, 'valid_all_auc.csv')

            self.file_names['test_all_aupr'] = os.path.join(
                args.model_name, 'test_all_aupr.csv')
            self.file_names['test_all_auc'] = os.path.join(
                args.model_name, 'test_all_auc.csv')
        ...
        self.best_valid = {'loss': 1000000, 'mAP': 0, 'ACC': 0, 'HA': 0, 'ebF1': 0, 'OF1': 0, 'CF1': 0,
                           'meanAUC': 0, 'medianAUC': 0, 'meanAUPR': 0, 'medianAUPR': 0, 'meanFDR': 0,
                           'medianFDR': 0, 'allAUC': None, 'allAUPR': None, 'concept_acc': 0, 'class_acc': 0}

        self.best_test = {'loss': 1000000, 'mAP': 0, 'ACC': 0, 'HA': 0, 'ebF1': 0, 'OF1': 0, 'CF1': 0,
                          'meanAUC': 0, 'medianAUC': 0, 'meanAUPR': 0, 'medianAUPR': 0, 'meanFDR': 0,
                          'medianFDR': 0, 'allAUC': None, 'allAUPR': None, 'epoch': 0, 'concept_acc': 0, 'class_acc': 0}
    ...
    def evaluate(self, train_metrics, valid_metrics, test_metrics, epoch, num_params, model, valid_loss, test_loss, all_preds, all_targs, all_ids, args):
        ...
        if args.dataset == 'cub':
            ...
            print('best Concept ACC:  {:0.3f}'.format(
                self.best_test['concept_acc']))
            print('best Class ACC:  {:0.3f}'.format(
